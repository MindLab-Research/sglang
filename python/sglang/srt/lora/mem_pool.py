...
class LoRAMemoryPool:
    ...
    def get_lora_A_shape(
        ...
        c = get_stacked_multiply(module_name, base_model)
        # MoE modules shard along `moe_tp_size`; shared MoE shards over full TP at
        # EP=1 (v0.5.16 fix), so it must not use moe_tp_size. Non-MoE row-parallel
        # modules use a probed shard that may be attn_tp under DP-attention.
        effective_tp_size = (
            self.moe_tp_size
            if self.is_moe_module(module_name)
            and not self.is_shared_moe_module(module_name)
    ...
    def get_lora_B_shape(
        ...
        # MoE modules shard along `moe_tp_size`; shared MoE shards over full TP
        # at EP=1 (v0.5.16 fix), so it must not use moe_tp_size. Non-MoE
        # column-parallel modules probe the OUTPUT axis
        # (quantization-independent). This used to call
