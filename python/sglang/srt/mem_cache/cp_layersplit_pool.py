...
class CpLayerSplitKVPool:
    ...
    def __init__(
        ...
        self._bcast_buf_latent = None  # reusable broadcast buffer for latent
        self._bcast_buf_indexer = None  # reusable broadcast buffer for indexer
    ...
    def broadcast_owner_layer_prefix(
        ...
        prefix_slots = (
            torch.cat(prefix_slots_list, dim=0).long()
            if prefix_slots_list
        ...
        # ---- Size-broadcast protocol -------------------------------------
        # All ranks MUST participate in the same number of NCCL broadcasts.
        # Previously, ``if not prefix_slots_list: return`` let some ranks skip
        # the broadcast entirely when their per-rank radix-tree match returned
        # no prefix (e.g. HiCache prefetch is local-only → tree can diverge).
        # That caused a permanent NCCL call-count mismatch → deadlock.
        #
        # Fix: broadcast the owner's element count first (1-element, all ranks
        # participate).  If the owner has 0 elements, every rank returns —
        # consistent.  Otherwise every rank broadcasts ``n`` elements.
        # NOTE: latent broadcasts token-level slots (n=len(prefix_slots));
        #       indexer broadcasts page-level unique rows (n=len(row_indices)).
        if kind == "latent":
            ref = self.transient_pool.kv_buffer[0]
            count = len(prefix_slots)
            row_indices = None
        elif kind == "indexer":
            ...
            row_indices = (
                torch.unique(prefix_slots // page_size)
                if len(prefix_slots) > 0
                else torch.empty(0, dtype=torch.long)
            )
        ...
        _n_tensor = torch.tensor(
            [count], device=ref.device, dtype=torch.int32
        )
        cp_group.broadcast(_n_tensor, src=owner_rank)
        n = int(_n_tensor.item())
        if n == 0:
            return  # owner has no prefix → all ranks agree to skip

        if kind == "latent":
            if self._bcast_buf_latent is None or self._bcast_buf_latent.shape[0] < n:
                if self._bcast_buf_latent is not None:
                    del self._bcast_buf_latent
                    torch.cuda.empty_cache()
                self._bcast_buf_latent = ref.new_empty((n,) + ref.shape[1:])
            buf = self._bcast_buf_latent[:n]
            if my_rank == owner_rank:
                buf.copy_(self.owned_pool.kv_buffer[owned_layer_idx][prefix_slots])
            recv = cp_group.broadcast(buf, src=owner_rank)
            if my_rank != owner_rank and len(prefix_slots) == n:
                self.transient_pool.kv_buffer[0][prefix_slots] = recv

        else:  # indexer
            if self._bcast_buf_indexer is None or self._bcast_buf_indexer.shape[0] < n:
                if self._bcast_buf_indexer is not None:
                    del self._bcast_buf_indexer
                    torch.cuda.empty_cache()
                self._bcast_buf_indexer = ref.new_empty((n,) + ref.shape[1:])
            buf = self._bcast_buf_indexer[:n]
            if my_rank == owner_rank:
                buf.copy_(
            ...
            recv = cp_group.broadcast(buf, src=owner_rank)
            if my_rank != owner_rank and len(row_indices) == n:
