...
class HiRadixCache(RadixCache):
    ...
    def check_prefetch_progress(self, req_id: str) -> bool:
        ...
        if min_completed_tokens <= 0:
            if info is not None:
                # NOTE: host_indices is lazily allocated on the operation
                # (only committed once L3 storage hits are known). When
                # completed_tokens == 0 it may be None; when > 0 it is set.
