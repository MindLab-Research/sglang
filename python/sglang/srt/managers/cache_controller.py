    ...
    def prefetch_thread_func(self):
        ...
        while (not self.storage_stop_event.is_set()) or not self.prefetch_queue.empty():
            ...
            try:
                ...
                # NOTE: no cross-rank all_reduce here. Local-only prefetch means
                # each rank queries its own pages, so hit counts are per-rank and
                # a per-op gloo all_reduce in this background thread deadlocks
                # against the scheduler's NCCL collectives (main thread).
