...
class DecodeRequest:
    ...
    # PD DSpark hidden state transfer fields
...
class DecodePreallocQueue(DecodeHiCachePreallocMixin):
    ...
    def _check_if_req_exceed_kv_capacity(self, req: Req) -> bool:
        ...
        if input_len > _capacity:
            message = f"Request {req.rid} exceeds the maximum number of tokens: {input_len} > {_capacity}"
    ...
    def _release_pd_hidden_rows(self, decode_req: DecodeRequest) -> None:
        """Release PD hidden receive rows for a preallocating request.

        The shared implementation lives on the transfer queue (which owns
    ...
    def pop_preallocated(
        ...
        for i, decode_req in enumerate(self.queue):
            ...
            if StateType.C128_STATE in state_types:
                # Prefer the all-ratio clear (c4 + c128, kv + indexer families):
                # a radix-hit decode bootstrap resumes the compressor
                # mid-sequence, and uncleaned c4 ring rows carry the previous
                # request's state on this slot (nondeterministic corruption).
                clear_state = getattr(
                    self.token_to_kv_pool, "clear_compress_req_state", None
                ) or getattr(
                    self.token_to_kv_pool, "clear_c128_req_state", None
                )
                if clear_state is not None:
                    clear_state(int(decode_req.req.req_pool_idx))
...
class DecodeTransferQueue(DecodeHiCacheTransferMixin):
    ...
    def _drain_pd_hidden_ready_chunks(self, decode_req: DecodeRequest) -> None:
        ...
        for chunk in sorted_chunks:
            ...
            if hidden_chunk.is_last_hidden_chunk:
    ...
    def pop_transferred(self, rids_to_check: Optional[List[str]] = None) -> List[Req]:
        ...
        for i, (decode_req, poll) in enumerate(zip(self.queue, polls)):
            ...
            if (
                ...
                if not (hidden_state.hidden_done and hidden_state.kv_done):
                    continue
