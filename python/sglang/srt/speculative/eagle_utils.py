...
def eagle_prepare_for_decode(batch: ScheduleBatch):
    ...
    for i, r in enumerate(batch.reqs):
        ...
        # [FIX-EAGLE] kv_committed_len also lags seq_lens by up to 2 iterations
        # under interleaved bs=1 batches, so committed + double_alloc can be <
        # seq_lens + next_n. verify's draft-token KV write (out_cache_loc =
