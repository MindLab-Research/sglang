...
class DSparkWorkerV2(BaseSpecWorker):
    ...
    def _forward_decode(
        ...
        if batch.spec_info is None:
            # PD-disaggregated first decode: seed bonus_tokens from the
            # prefill's produced first token (already appended to output_ids by
            # the prefill side), NOT the prompt's last input token. The input
            # token differs from the sampled continuation and corrupts the first
            # DSpark draft step (garbled/repeated output).
            try:
                ...
                logger.warning(
                    "DSpark PD first-decode seed fallback failed: %s (origin_input_ids=%s)",
                    e,
                    [getattr(r, "origin_input_ids", None)[-3:] for r in batch.reqs],
                )
            ...
            batch.spec_info = DFlashDraftInputV2(
                topk_p=torch.empty((0, 0), device=self.device, dtype=torch.float32),
                topk_index=torch.empty(
                    (0, 0), device=self.device, dtype=torch.int64
                ),
