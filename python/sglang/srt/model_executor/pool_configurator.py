...
class DefaultPoolConfigurator(MemoryPoolConfigurator):
    ...
    def __init__(self, kvc: KVCacheConfigurator):
        ...
        if is_cp_layersplit_active(kvc.server_args, kvc.ps.attn_cp_rank):
            num_layers = cp_layersplit_local_layer_count(
                num_layers, kvc.server_args.attn_cp_size, kvc.ps.attn_cp_rank
            )
        ...
        if (
            ...
            if (
                ...
                # physical token.
                draft_token_multiplier = max(1, kvc.server_args.dcp_size)
                self._cell_size = int(
                    self._cell_size
                    * (
                        1
    ...
    def __init__(self, kvc: KVCacheConfigurator):
        ...
        if (
            ...
            if draft_layers is not None and int(draft_layers) > 0:
                ...
                # DCP: draft stays unsharded while the target stores one page
                # shard per DCP rank; budget dcp_size copies of draft tokens.
                self._draft_token_multiplier = max(1, kvc.server_args.dcp_size)
