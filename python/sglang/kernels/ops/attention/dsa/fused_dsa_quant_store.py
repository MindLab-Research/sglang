...
def _fused_dsa_quant_store_kernel(
    ...
    if block_id < NUM_NOPE_BLOCKS:
        ...
        # NOTE: MUST match quant_k_cache._quantize_k_cache_fast_kernel EXACTLY.
        # Use the same scale-inverse multiply (not division) and FP8_MIN/FP8_MAX
        # bounds, otherwise floating-point rounding differs (y / y_s vs
        # y * (1.0 / y_s)) and the write is NOT byte-identical to the unfused
        # path on this branch (B300 + triton 3.6.0). Verified by test_correctness.
...
def test_correctness() -> bool:
    ...
    IMPORTANT: uses a COLLISION-FREE loc (``torch.randperm``), not the naive
    ``randint``. A random ``randint(0, size, (num_tokens,))`` with
    ``num_tokens=128`` over ``size=256`` has ~27 duplicate slots; for a
    collided slot the "last writer wins" differs between the torch reference
    (serial overwrite) and the triton fused kernel (parallel, undefined order)
    -- so the naive test spuriously FAILS even though the fused kernel is
    lossless. This was the root cause of the original test_correctness failure
    on this branch (B300 + triton 3.6.0).
    """
    ...
    # Collision-free slot ids: every token writes its own slot.
    loc = torch.randperm(size, device="cuda")[:num_tokens].to(torch.int32)
