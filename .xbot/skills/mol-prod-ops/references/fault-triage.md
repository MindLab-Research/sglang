...
| **1100 standalone MoL**（2026-09-02 实录）: 18777 全 503（router `smg_router_standalone.log` 持续 503，latency ~400ms）但 1101 router/gateway/proxy 进程全活；1100 无进程、GPU 空闲、30200/30100 无监听；引擎日志末尾 8×TP `OSError: [Errno 28] No space left on device`（`triton/compiler/compiler.py make_cubin fsrc.write`）| **standalone 引擎启动缺 HiCache 三件套 env → 落盘写 /tmp/hicache（30G /dev/vda）**（`--file-storage-path` 单独不生效，与 `mol-sglang-hicache-tmp-symlink` 同族）+ `/tmp/adapter` 15G 陈旧 LoRA → 盘满 → Triton JIT tempfile ENOSPC → 齐崩 19h 无人拉起 | ①`rm -rf /tmp/adapter /tmp/hicache` ②`mkdir -p /root/hicache && ln -s /root/hicache /tmp/hicache` ③带 env `SGLANG_HICACHE_FILE_BACKEND_STORAGE_DIR=/root/hicache`+`MAX_SIZE=200G`+`MIN_FREE_SPACE=10G` 重启（恢复脚本 `/root/start_standalone_mol.sh`，server_args 逐项对齐）；1101 router/gateway/proxy 带 `--disable-circuit-breaker` **自动恢复无需重启**（实测引擎就绪 ~1min 后 503→200） | `mol-1100-standalone-enospc` |
