...
- 相关环境变量（启动 router 时）：`SMG_JOBS_DIR`（默认 ./smg-jobs）、`SMG_JOBS_MAX_CONCURRENCY`（默认 64）、`SMG_JOBS_REQUEST_TIMEOUT_SECS`（默认 3600）、`SMG_JOBS_SELF_URL`（默认 `http://127.0.0.1:{port}`）、**`SMG_JOBS_ENGINE_URLS`**（逗号分隔的引擎地址，`continue_from` 续跑把源 prompt 还原成 token id 时用；不设则回退到 router 启动参数里的 worker URL，见 §10）
- 当前部署命令（B300-1）：
  ```bash
