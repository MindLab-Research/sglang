...
> **周期**：2026-08-13 → 2026-08-24（11 天连续工程）
...
> **最终验收**：legacy 与窗口模式双通过——case50 ×4 轮 @600rpm 全绿（42/50 + 8 例已知
> DFlash grammar 限制豁免）、内容零污染、单轮 <2.5min、0 崩溃 0 泄漏、21 请求并发中止
> 风暴存活、523,099 次集合通信 × 8 rank 位点序列完美奇偶
...
### 3.4 战役：窗口模式"未修 bug"冤案平反（2026-08-24，四层根因链）

R1 事故后"窗口 streaming 模式有跨请求 chunk 乱序未修 bug、生产一律 legacy"的结论
被推翻——R1 实为四层叠加：

| # | 根因 | 修复 |
|---|---|---|
| 1 | start 脚本 export 误置于 launch **之后**——窗口模式从未真正生效（所有"验证过的 legacy"其实一直跑 legacy） | env 前移 + 进程 env 双重确认 |
| 2 | 控制面 zmq socket **永久黑洞**：`RECONNECT_IVL=-1` + monitor 只订 DISCONNECTED——断线轮换后新 socket 连接失败即永死（L4 下 5/8 rank 对集体失明） | `RECONNECT_IVL=100` + 双事件 monitor 按最后事件判定 |
| 3 | notify 丢失无自愈（LINGER=0 轮换本身丢在途消息） | 协议三件套：3s 重发 + 去重补 ACK + 缺口等待 60s 兜底 |
| 4 | park-behind 唤醒竞态（孤儿 chunk 永不通知） | 原子 check-and-park + 单锁段 pop&wake |

**架构启示（"自愈是不是 hack"之问的答案）**：第 2 层是真缺陷（已根源修复）；第 3 层
的 LINGER=0 丢消息是**有意的传输设计**（快速死节点检测的代价）——应用层"重发+去重"
是不可靠信道上关键消息的教科书协议（TCP 同款：重传+幂等去重），非 hack；健康路径
零开销（gap=0 的轮次重发从未触发）。"换确认送达传输层"的纯度路线会引入阻塞语义
（对端死亡时发送方卡死）——正确架构 = **有损快速信道 + 应用层 exactly-once 效果**。

**验证矩阵**：ladder ×3（冷/热树）全 20/20、case50 ×4 轮全绿、**21 请求同时强制中止
风暴存活**（比 R1 诱因大 3.5 倍）、**523,099 次集合通信 × 8 rank 位点序列完美奇偶**
（PADDED-AR 逐调用插桩，任何漂移即刻定位到 rank+位点）。

### 3.5 战役：decode "断崖"终审——一个不存在的 bug（负结论的方法论价值）

用户观察到"decode batch 间隔随输出长度断崖式下跌"，疑似随长输出（>10K token）恶化。
四重判别实验层层证伪，最终结论：**断崖 bug 不存在**。

| 实验 | 结果 | 证伪对象 |
|---|---|---|
| 280K 单请求长输出 | 16.1→19.5ms 平稳线性漂移（+21%），accept 恒 6.00，无 rebootstrap | 单请求无长度断崖（至 280K ctx） |
| 75K"断崖"复盘 | 04:41-04:47 有 11 个外部请求（38-68K prompt）穿插污染：bs=2 步时 +22%、日志 accept 是 **batch 平均值**（同伴拉低显示值）、1.9s 卡顿=大传输 batch_shard_all | "64K dense→sparse 过渡"理论 |
| 280K 运行中短请求穿插 | accept 6.00→5.98→6.00 零损伤 | merge/split 腐蚀理论 |
| 生产 GLM bs 分组判别（24665 batch） | bs=1 平坦到 64K、bs≤4 平坦到 **240K**、bs≥5 恒 75ms | 生产长度断崖（bs 混杂假象） |

**现象的真身（三重良性解释）**：①并发 scaling——长输出请求占用槽位更久 → 在途堆积 →
bs 涨 → 步时随批注意力成本涨（**5× 批仅 1.5× 步时，实为良好扩展性**）；②投机预热
瞬态——请求前 ~200 token accept 从 3.2 爬到 6.0；③V4 indexer 线性漂移——每步扫描
O(ctx/128) block 的预期 scaling。

**方法论沉淀**：(a) **batch 级指标是平均值**——bs>1 时单请求的 accept/throughput
不可从 Decode batch 日志读出（同伴请求拉低显示值是经典陷阱）；(b) **bs 必须分组
分析**——任何按 #token 分桶的步时统计都被并发度混杂（长输出 ⇔ 高 bs ⇔ 慢步时，
相关≠因果）；(c) **单变量现场复现**——在长请求运行中插入可控的第二请求，直接观察
目标请求的反应；(d) **测量环境污染排查**——"断崖"出现时先枚举窗口内全部请求
（PDH-ADMIT/DCP-PD-IDX），排除外部流量巧合。

**可选优化（非 bug，性能机会）**：indexer 增量打分（缓存前缀 block 分数复用，抹平
+21% 漂移）、批注意力调度优化。

---

## 4. 最终验证（2026-08-23/24 双轮，1102/1104 测试对，legacy + 窗口双模式）

### 4.1 正确性验收（legacy 模式，2026-08-23）
...
### 4.2 窗口模式（RECV_WINDOW=16384）终验（2026-08-24，commit `35fa068e33`）

| 验收项 | 结果 |
|---|---|
| ladder ×3（冷/冷/热树，含 42 并发 L4） | 全部 **20/20 probes clean**；热树 L4 76.2s |
| case50 ×2 @600rpm（R47/R48） | 均 42/50+8 grammar，5/5 clean，~2m20s |
| **21 请求同时强制中止风暴** | 引擎存活、探针 4.6s/0.7s 快速恢复、零漂移 |
| **集合通信奇偶（PADDED-AR 插桩）** | **523,099 次调用 × 8 rank 位点序列完全一致** |
| 协议指标 | ooo=0 / gap=0 / park=0 |
| 长输出（280K token 单请求） | accept 恒 6.00、无 rebootstrap、16.1→19.5ms 线性漂移（+21% 属 indexer O(ctx) 预期 scaling） |
...
- **~~窗口 streaming 模式的跨请求 chunk 乱序 bug 未修~~** → 已于 2026-08-24 彻底
  修复并全量验收（见 §2.3/§3.4）：四层根因（脚本 env 位置 / socket 黑洞 / notify
  丢失 / park 竞态）全部结案，21-abort 风暴存活 + 523K 集体调用奇偶校验通过。
- **~~decode 步时随长输出断崖式下跌~~** → 2026-08-24 终审：**bug 不存在**（见
  §3.5）。单请求平坦至 280K ctx；现象=并发 scaling（5×批仅 1.5×步时，良好扩展）
  + 投机预热瞬态 + indexer O(ctx) 线性漂移（+21%@280K，预期）。
...
- 关键 commit：`1910bb619c`（窗口准入）、`98e7a09e9e`（钳制复活五件套）、
  ...
  `35fa068e33`（窗口模式四层修复：socket 自愈 + 重发/去重/缺口容忍 + 原子 park）
- 判据工具：`contam_check.py` / `lc_ladder.py`（内容金丝雀与判别阶梯）、
  `longprobe.py`（长输出逐 chunk 延迟剖析）、`padded_seq_diff.py`（集合通信
  位点序列奇偶校验）、`abort_storm.py`（并发中止风暴），节点 `/tmp/bench_/`
