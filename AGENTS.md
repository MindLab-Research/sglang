...
- **⛔ 乱码/短循环根因定位（2026-08-16 03:0x 决定性复现）：radix 命中路径的非确定性脏读，CP 已被排除**：最简复现 = **同一请求单发串行 ×4（temp=0、cached 完全相同 139264/139734）→ 3~4 种不同输出**（1 干净 1 循环 2 同款循环，首个 token 即分叉）。greedy 同输入同命中必同输出 → 服务路径读到**随上次请求残留而异的未初始化/陈旧槽位**。首个输出 token 由 prefill 的 extend forward 产生 → **方差在 prefill 侧 delta 重算 forward**（470 token delta 分配 512 槽=2 页，42 个尾槽为池残留；DSA indexer/compressor 若按页粒度读即混入他请求 KV → 输出复读 prompt 片段 `_CTRL_`/`abort_`）。已排除：CP（摘除后 8 并发仍 9/50 循环）、radix OFF/L3 清空、温度、单请求多 chunk（373K 单发干净——因为全量重算写入全部槽）。**CP（d300）被冤枉，勿急着恢复**；修复方向 = 命中路径的确定性（分配尾槽清零 or indexer 严格 seq_len 边界），需先插桩定位具体子系统（indexer topk hash / 首 token logits hash 对比）。判据工具：dev 47.87.64.67 `/tmp/nondet.py`（4 连发分叉计数）、`/tmp/diverge.py`（单发 vs 并发首分叉位）。
