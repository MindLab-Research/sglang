...
| `docs/agent/dcp-virtual-id-domain-fix.md` | **DCP 虚拟 id 域终极修复（2026-08-20，`9db63a6abb`）**：EAGLE accept-cliff + Xid 31 同家族根因——decode allocator 虚拟空间（page_size=64×dcp, capacity=size×dcp）下 req_to_token 存虚拟 id，`3c68f20891` 的 prepare_for_draft global→local 修复 = **双转**（set_mla_kv_buffer DCP 分支再转一次）+ sanitize 破坏 + 读路径无 rank 过滤 → 写错位/Xid 31/跨 rank 污染；修复=保持虚拟 id 原样（写路径单转、读路径 rank 过滤 `(id//64)%dcp==rank`、free 用虚拟 id）。公式通用 DCP=4/8。判据：DRAFT-LOC-OOB/FOREIGN=0、accept 单请求 ~5.9 / 64 并发 ~3.2 |
