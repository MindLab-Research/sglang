...
- **⛔ 乱码/短循环根因【已锁定 2026-08-16 11:2x 插桩实锤】：DSV4 compressor 链式状态在 radix 命中时读到 rpi 槽残留（c4_state 环无初始化、树节点状态无恢复路径）**：证据链：① 最简复现——同一请求单发串行×4（temp=0、cached 全同 139264/139734）→ **4/4 输出互不相同**（含 1 循环）；全量重算（cached=None）run 干净；② ALLOC-DIAG 插桩显示 3 个命中 run **rpi=4/5/6、KV 槽各异、输出各异**（状态环按 rpi 位置寻址→残留随 rpi 历史变化→非确定）；③ `clear_c128_req_state`（memory_pool:1004）证明状态行是会被读的、需初始化——但它 `ratio != 128: continue` **跳过 c4**，且只在 decode bootstrap 调用；④ **full_component.py 无任何 c4/compress-state 恢复**——树按节点存了 c4_state（hicache `*.c4_state_*.bin`）但 match 时从不拷回请求位置环 → prefill 命中后续压 delta 的 carry = 上一任请求残留 → 压缩 KV 错 → attention 读错 → token 级循环/提前停。**修复设计**：match post-processor 增加 compress-state 恢复（树节点 state 行 → 请求 rpi 环行，参照 swa_component._restore_device_value 模式）+ c4 侧补 clear（对齐 c128）；两头（prefill+decode）都要。**CP（d300）已洗清嫌疑**（CP off 仍复现），修好状态恢复后可安全恢复 CP。复现/判据工具：dev 47.87.64.67 `/tmp/nondet.py`（4 连发，修好后必须 4/4 一致）、`/tmp/diverge.py`；插桩：prefill `SGLANG_DEBUG_ALLOC=1`（schedule_batch.py [ALLOC-DIAG]）。
