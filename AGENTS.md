...
- **⛔ MoL 多 LoRA 乱码 = 三层叠加（2026-08-21 全部结案）**：①cuda graph stale mapping + 驱逐（`f070d3d466`，上游 #29468 移植）；②HiCache host 层 extra_key 污染（生产已禁 HiCache）；③**真根因：prefill-CP pad 行被施加 LoRA delta（`12cda1cd60`）**——round-robin shard 把 pad 行归属最后请求 → delta 把 pad 激活打成 NaN → pad KV 槽 NaN → 下层 attention 真实 query 中毒 → logits NaN → argmax=0 乱码。base/CP-off 恒净、LoRA+CP-on 乱（filler prompt 75-100% 复现）。修复=pad 行映射 adapter slot 0（base 槽 rank0=no-op）。⚠️ segment 结构改动必须保持 covered==x_rows 不变式（排除行→permutation 短于 x→absorbed step kernels IMA）。**乱码排查铁律：双端全新重启后测（污染状态毁对照实验）；"组合才坏"用 SGLANG_LORA_FAMILY 运行时二分；逐层 dump 行级 NaN 分析（pad vs 真实行）是金标准；验证 decode 真加载用 `lora_pool_slots_used`≥1**。完整链见 `docs/agent/lora-multi-adapter-garbling.md`。
