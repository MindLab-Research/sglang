...
- **⛔ MoL 多 LoRA 乱码 = cuda graph stale mapping + 驱逐（2026-08-21 修复 `f070d3d466`）**：base+4 adapter 超 `max_loras_per_batch=4` 触发 LRU 驱逐后，`token_lora_mapping` 的 CG buffer 尾部保留 stale adapter id（只在 DP 场景才清尾）+ rank-0/base 映射 slot 0 而非 -1 → 回放时 padding/base 行路由到错误虚拟专家 → 乱码且 sticky 传染（驱逐后 adapter 可占 slot 0，base token 静默吃 delta）。上游 #29157/#29468 同源（closed 未合并，本修复即其移植）。**乱码排查铁律：必须双端全新重启后测（污染状态毁对照实验）；验证 decode 真加载 LoRA 用 `lora_pool_slots_used`≥1 + 8 行加载日志，别用"输出与 base 不同"判断（prefill 承担了全部可观测差异）**。完整结案见 `docs/agent/lora-multi-adapter-garbling.md`。
