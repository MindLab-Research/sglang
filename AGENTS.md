...
- **⛔ 乱码/短循环（2026-08-16 彻夜排查，嫌疑=CP 混合 batch，生产已隔离）**：64 并发 replay accept 爬到 ~1.0 + token 级循环（`_CTRL_`/`abort_`/`glad_` 等 prompt 片段复读）。**复现矩阵**：① 单请求任何长度（含 373K 多 chunk）/温度/命中态——干净；② 并发下**固定行**循环（>16K tokens 长行高发，行集合随 batch 组成变）；③ radix OFF / L3 清空 / temp=1.0 / SWA 窗口钳位（`ce98ae3720`）均不消除。**嫌疑：prefill CP（d300）混合 batch round-robin 分片/重组**——生产暂摘 `--enable-prefill-cp` 隔离（CP 代码与 3.5× 加速保留在 d300afc750，修好后一行恢复）。**排查教训**：① case50 `+++++++` 循环是 git diffstat 误报——判定必须"含词语 pattern 且占输出主体"；② replay CSV 前 50 行是同一乱码文本截断变体，temp=0 有天然循环倾向，**判污染必须 temp=1.0**；③ dev 服务器 47.87.64.67 有 replay.csv，/tmp/replay_dump.py 可复刻+dump；④ router `cache_aware` 与 prefill radix OFF 不兼容（503 卡死）再确认。
