...
| `docs/agent/hisparse-pd-radix-plan.md` | **hisparse × PD × radix 打通实现计划（2026-08-24）**：7 个 Phase（prefill host 池/两端域一致/准入预算/bs64 不降只缩 device 池/radix host-only/资源释放/端到端验证）。含 Phase 0 实证（prefill host 池 24GB 已建但 `prefill.py` 不用→OOM）。每 Phase 标了改动文件+行+验证点+风险。**尚未执行**，待审查后开工 |
