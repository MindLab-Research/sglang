...
- **⛔ Triton constexpr→runtime 消除 runtime JIT 编译死锁（2026-08-31 修复 `4f9a27c601`）**：`transform_index_page_table_decode_kernel` 的 `PAGE_TABLE_LEN`/`page_table_row_stride`/`KV_POOL_CAP` 是 `tl.constexpr`——每个新 batch shape（不同 max_seqlen_k）触发 Triton JIT 重编译。8 rank 同时 cache-miss → 同时 `_init_handles`（compiler.py:466）→ 编译锁竞争 → rank 0 先完成进 `run_eagle_verify` NCCL all_gather，rank 1-7 卡在 JIT → **互相等 = 死锁**（22 分钟，health 200，py-spy：7 rank 在 triton/compiler + 1 rank 在 run_eagle_verify）。与 tilelang JIT 并发编译死锁（2026-08-15）**同族**——多进程 runtime JIT + collective 交叉 = 必死锁。**修复=改 runtime 参数**（去掉 `tl.constexpr` 注解，不再触发重编译）。**如何彻底避免（新 kernel 编写铁律）：① Triton kernel 的 shape 依赖参数（页表长度/stride/池容量等随 batch 变化的值）一律用 runtime 参数，不用 `tl.constexpr`——只有常量（TOPK=2048、BLOCK_SIZE）才用 constexpr；② 新 kernel 首次调用必须 warm-up 全部常见 shape（启动时用 dummy tensor 触发编译，把 JIT 移出 serving 路径）；③ 任何 per-rank 差异化的 JIT（tilelang/Triton/DeepGEMM）都可能造成编译时序分歧 → collective 错位——检查 kernel 是否 rank-invariant（同 shape 同编译产物）。prefill 孪生 kernel（`transform_index_page_table_prefill_kernel`）同修（strides + PAGE_TABLE_LEN 全改 runtime）。判据：py-spy 7 rank 在 `triton/compiler/compiler.py:466 _init_handles` + 1 rank 在 NCCL = 此病。
