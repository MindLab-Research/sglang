...
- **⛔ DSPARK accept rate 接近 1 + token 级短循环 = greedy × 退化 prompt 的模型行为，不是 KV 污染（2026-08-15 彻查结论）**：replay.csv 乱码行 + 请求体自带 temp=0 → 模型分布尖锐锁进循环吸引子（`CTRL_`/`abort_`/`OPTIONS_` 等 prompt 片段复读），DSPARK draft 完美跟随 → accept 0.99。**KV/分布本身正常**（temp=1.0 下同 prompt 零循环）。二分已穷尽：radix OFF/CP OFF/串行单请求均复现（串行 50% 循环率）→ 与 radix/CP/并发全无关。判据：**accept 异常高 + 输出短循环** = 模型循环；真 KV 污染时 target 分布变随机、draft 不跟随 → **accept 反而掉**。修复：mint-bench `replay_temperature` override（bee3355）；64c replay temp=1.0 验收 accept 0.12-0.19、length 锁死 63/204→1/182、DSP-ACCEPT 循环 61%→0%。详见 docs/agent/deepseek-v4-pro-summary.md §6。
- **⛔ router `--policy cache_aware` 与 prefill radix OFF 不兼容（2026-08-15）**：二分实验关 radix 后 router 对 prefill 健康探测持续失败 → 503/卡死（一晚三次）。关 radix 做实验必须同时 router 换 `--policy round_robin`。另：decode 去 `--speculative-algorithm DSPARK` 起不来（PD bootstrap 依赖 spec_metadata 协商，`Failed to get kvcache from prefill`），不能用关投机解码做二分。
