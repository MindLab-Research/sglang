...
- **⛔ prefill 的 DG_JIT_NVCC_COMPILER 必须用 wrapper（2026-08-15 修复）**：prefill 曾直接指向裸 nvcc，缺 `--expt-relaxed-constexpr` → 16 并发新 M shape 触发 `tf32_hc_prenorm_gemm` DeepGEMM JIT 编译失败（`NVCC compilation failed`, compiler.hpp:228）→ prefill crash → decode 端表现为 `Attempting to reconnect to 10.0.0.75:8998...`（bootstrap 断连是**后果不是原因**）。**修复**：两端 `DG_JIT_NVCC_COMPILER` 都指向 `nvcc_expt.sh`（`exec <venv>/nvidia/cu13/bin/nvcc "$@" --expt-relaxed-constexpr`）。排查此类问题时先看 prefill 日志的 `Scheduler hit an exception` traceback，不要被 decode 端 reconnect 日志误导。
