...
| `3df13dd42` | fix(prefill)：**iteration barrier 移到 `pop_bootstrapped` 前**——修（有在飞请求停滞期间 py-spy 观测到的）`poll_and_all_reduce_attn_cp_tp_group` all_reduce 死锁（utils.py:138 vs 143 行号不一致 = collective 不匹配） | disaggregation/prefill.py |
...
### 5.5 GLM-5.3 2P2D + L3 store（38.255.28.6/.7/.8/.9，2026-08-30 重部署）

B200 容器集群：`.6/.7` prefill（30100，CP=8 layersplit + HiCache L3 mooncake）+ `.8/.9` decode（30200，DCP=8 + EAGLE + radix + HiCache L3）+ `.6` router（30000，cache_aware，key 见 secrets.env）。**L3 = mooncake store 集群（TCP 无 RDMA）**：master `.6:61051/61080` + 4×store daemon（137GB 段 + 4TB SSD offload）。代码基线 `0b589a9b19`；部署/恢复一律用 `deploy/glm53-2p2d/deploy_2p2d.sh`（storefix/storecreate/prefill/decode/router 五分支 + tar overlay）+ `docs/agent/glm53-2p2d-deploy.md`（runbook + 5 坑）。1102/1104（B300 裸机对，GLM-5.3 PD 测试）：`/root/start_glm53_{prefill,decode}.sh` + `/root/v15_patched` venv（L3=file backend）+ smg router `1102:31000`；代码 = tar 覆盖 venv srt/ + 清 pycache。**⛔ 引擎容器 mooncake env 一个都不能少**（`MOONCAKE_PROTOCOL=tcp` + `MC_FORCE_TCP=1` + `MOONCAKE_TE_META_DATA_SERVER=http://38.255.28.6:61080/metadata` 等 22 项 common_env）——缺了 = store 复用条件走默认 rdma → `InnerTransferEngine` cast 崩（2026-08-30 一次坑 4 节点）。store daemon 三坑：CMD 必须**单字符串**（数组形式 bash 只执行 "exec" 秒退 exit 0）、必须 `--gpus all`（否则 libcuda ImportError）、启动前必须 mkdir offload 目录（否则 FileStorage ValueError exit 1）。
...
- **⛔ PFX-SYNC：跨 rank prefix min 协商——rank 分歧的源头消灭（2026-09-03 部署，`3812e4febb`）**：per-rank radix 分歧（L1 驱逐/L2 write-back 时序 per-rank）过去在传输层处理（PD-PFX-MIN min-of-8 + offset/TRUNC/MISMATCH，c9963809ce）——必要但治标：每条处理路径长出自己的残余 bug（offset 下游 IndexError WERR、空dst MISMATCH 503，分歧风暴 ~1200 room/h 下 ~2-4% 请求失败）。**本修复消灭分歧本身**：decode 8 rank 在任何 ACK（send_metadata）之前集体协商 min(total_prefix_len)——`_pfx_sync_collective`（一次 all_gather：per-room (room, cached_total) 对；room 在全部 rank 的 waiting 集且 8 个 cached total 齐 → GRADUATE（pfx_sync_min 置上，STICKY））+ 两阶段 prealloc（tick N 匹配+缓存（锁持有）+延迟；tick N+1 毕业后钳位+ACK）+ 钳位（own > min → prefix_indices 切到 min，l2/l3 清零，分歧段 [min, own) 从 prefill 重收 ~40MB/事件）。8 个 ACK 携带同值 → 传输层永远看不到分歧 → offset/TRUNC/MISMATCH 全部变死代码（保留为安全网）。**budget 断裂锁保留**（毕业请求的 cached match 锁跨 budget 等待持有——释放=页可驱逐=下 tick 缓存 match 指向已驱逐页=KV 损坏）+ FINISH_ABORT 扫描释放缓存锁（泄漏修复）+ 补偿路径（retracted 提前返回 + polling else 调空协议保持 gloo 计数对齐）。+1 tick/请求延迟（invisible：prefill bootstrap 本就串行最慢 ACK）。判据：`[BS-T] prealloc-done ... pfx_sync(min=X own=Y)`（8 rank 同 min）、`[PFX-SYNC] graduated/clamped`（钳位=分歧被源头消化）、prefill 的 `PD-PFX-MIN` **永不再触发**（ACK 均匀）。
...
- **⛔ HiCache L2 write-back 驱逐 hasattr 3 参数崩溃（2026-08-30 修复 `0b589a9b19`）**：`memory_pool_host.py::backup_from_device_all_layer` 的 `_host_cap` 行 `hasattr(self,"index_k_with_scale_buffer",None)`（Python hasattr 只收 2 参数）→ L2 池满 evict 触发时 TypeError → scheduler 崩 SIGQUIT（1102 prefill 实录，prefill 运行数天后池满才爆）。`22d8d3763b` 只修了 staging gate 同款笔误，此行漏了。判据 `grep "hasattr expected 2" prefill.log`。修复已部署全部 6 节点（B200 tar overlay + 1102/1104 venv 覆盖）。GLM-5.3 2P2D 部署 runbook 见 `docs/agent/glm53-2p2d-deploy.md`（含 mooncake env 全集=cast 崩根因、store daemon 三坑：CMD 单字符串/`--gpus all`/mkdir offload）。

- **⛔ MoL（虚拟专家）LoRA stage 配置的 padding 陷阱（2026-09-15，flag `SGLANG_OPT_MOL_LORA_STAGE_CFG`）**：`virtual_experts.py` 的 shrink/expand 复用 MoE stage config，而这些形状（`num_experts = max_loras × n_routed`、`K = rank`）**无 autotune 条目** → fallback `BLOCK_SIZE_M=64 / BLOCK_SIZE_N=64`；`_get_routing` 又把 `BLOCK_SIZE_M` 交给 `moe_align_block_size` ⇒ 每个被触及的虚拟专家桶 pad 成整 64 行 ⇒ **padded rows ≈ 1604×64 ≈ 1e5（真实 pair 仅 ~1.1k，~93× 膨胀）**，且 `grid = blocks × cdiv(N, BLOCK_SIZE_N)`。实测（bs=23，60 步 profiler，kernel-only/TP0）：down-delta expand `grid=[153984]` **160.6 µs/层 ≈ 14% 步时 @ ~85 GB/s ≈ HBM 峰值 1%**，而同层 gate_up expands `grid=[6416]` 仅 7.5/6.8 µs（N 比 6144:256 = 24 恰好 = grid 比 24 ✓，另 shrink `grid=[1604]` 也对账）。**判据**：读 profiler 的 `fused_moe_kernel` `args.grid`，down expand 若 ~1.5e5 即此病（`args` 里 size 稳定 + `gzip -t` 后再解析）。**修法**：flag 打开后只把 `BLOCK_SIZE_N` 调到 `min(256, 该 stage 真实 GEMM N)`（grid 4×↓，**逐位等价**——`BLOCK_SIZE_K` ≥ rank 不变、expand 无 split-K、每个输出元素仍单 tile 内完成 K 归约）；**⛔ 不要动 `BLOCK_SIZE_M`**：host 端 `grid_m = cdiv(sorted_token_ids.shape[0], BLOCK_SIZE_M)`，而 `sorted_len` 是最坏情况**分配**长度（`numel + virtual_E×(BLOCK_M−1)`）⇒ `grid_m ≈ virtual_E + numel/BLOCK_M`，对 BM 只有**弱依赖**（几个百分点），却让每个 tile 多走 padding 行。shrink（rank≤64）在此 flag 下是 no-op，其 48.3 µs/层属另一问题（小 tile + 长 K-loop）。
- **⛔ Triton constexpr→runtime 消除 runtime JIT 编译死锁（2026-08-31 修复 `4f9a27c601`）**：`transform_index_page_table_decode_kernel` 的 `PAGE_TABLE_LEN`/`page_table_row_stride`/`KV_POOL_CAP` 是 `tl.constexpr`——每个新 batch shape（不同 max_seqlen_k）触发 Triton JIT 重编译。8 rank 同时 cache-miss → 同时 `_init_handles`（compiler.py:466）→ 编译锁竞争 → rank 0 先完成进 `run_eagle_verify` NCCL all_gather，rank 1-7 卡在 JIT → **互相等 = 死锁**（22 分钟，health 200，py-spy（**有在飞请求停滞期间**，见 §8 诊断铁律）：7 rank 在 triton/compiler + 1 rank 在 run_eagle_verify）。与 tilelang JIT 并发编译死锁（2026-08-15）**同族**——多进程 runtime JIT + collective 交叉 = 必死锁。**修复=改 runtime 参数**（去掉 `tl.constexpr` 注解，不再触发重编译）。**如何彻底避免（新 kernel 编写铁律）：① Triton kernel 的 shape 依赖参数（页表长度/stride/池容量等随 batch 变化的值）一律用 runtime 参数，不用 `tl.constexpr`——只有常量（TOPK=2048、BLOCK_SIZE）才用 constexpr；② 新 kernel 首次调用必须 warm-up 全部常见 shape（启动时用 dummy tensor 触发编译，把 JIT 移出 serving 路径）；③ 任何 per-rank 差异化的 JIT（tilelang/Triton/DeepGEMM）都可能造成编译时序分歧 → collective 错位——检查 kernel 是否 rank-invariant（同 shape 同编译产物）。prefill 孪生 kernel（`transform_index_page_table_prefill_kernel`）同修（strides + PAGE_TABLE_LEN 全改 runtime）。判据（**须有在飞请求停滞**，见 §8 诊断铁律；空闲态线程等待不算）：py-spy 7 rank 在 `triton/compiler/compiler.py:466 _init_handles` + 1 rank 在 NCCL = 此病。
- **⛔ paged allocator off-by-one 超域页→跨请求 KV 污染（2026-08-31 终局 `59c8532d79`）**：`PagedTokenToKVPoolAllocator.clear()` 的 `arange(1, num_pages+1)` **多出一页**（页 num_pages，首槽 = num_pages × page_size = 池 cap 精确咬合 OOB samples[0]）→ 超域页进 free list → 分配时发出 cap 及以上槽位 → guard 洗 0 → 100 段 KV 写共享 padding 页 0 → **跨请求互读互写 = 确定性 KV 污染**（.8 生产 03:04 UTC：100/99940 OOB slots，后续受害者 accept 塌至 0.07 + 数字汤 + 重试不自愈）。修复=clear 上界改 `num_pages`（去掉 +1）+ free 的 `<=` 改 `<`（回收路径同修）+ guard 洗 0 改 fail-fast（raise RuntimeError 走单请求 abort，不再静默写页 0 毒全池）。单测 4/4 PASS（排空 17207 页 max slot=cap-1 无越界）。判据：`KV-PRODUCER-OOB` 应为 0；`PADDED-AR-DIVERGENCE` 应为 0。完整见 `docs/agent/decode-hicache-read-before-ready.md`。
- **⛔ read-before-ready：decode HiCache L2 load_back 树 value 提交先于 DMA（2026-08-31 `7da1b4e9b8`）**：`load_back()` 在 DMA 排队后**同函数内** commit 树 value（full_component.py:327 `cd.value = device_indices`）→ 节点 evicted→False 翻转 → match 立即可见 → **并发请求 match 命中 pending 槽位视为 L1 命中 → forward 读未完成 DMA**。修复=forward stream 级 wait-all-in-flight（`LayerDoneCounter.wait_all_in_flight(model_runner.forward_stream)`，在 `tp_worker.set_hicache_consumer` 的 `consumer_index < 0` 分支调用——每 forward 一次 gate 该 stream 上所有后续读）。**decode HiCache 乱码三重叠加的另一层**：guard 洗 0（off-by-one 修复处理）+ read-before-ready（本修复）+ Triton 编译死锁（本修复附带消除的时序分歧源）。
- **⛔ PD KV 传输 src/dst 错位 = decode 8 TP rank radix 命中分歧（2026-09-02 结案，`docs/agent/pd-rank-divergence-root-cause.md`）**：decode 每个 TP rank 独立 `_match_prefix_and_lock`，`decode_prefix_len = L1_hit + L2_host_hit`（decode.py:1263，**l3 故意排除**）是 per-rank 本地状态——L1 驱逐由本 rank `available_size()` 水位驱动（各 rank 碎片/在途不同→驱逐受害段不同）、L2 write-back（`write_back` 策略）异步完成时序 per-rank、L2 满后 L2→L3 逐出 per-rank → **同一请求 8 个 ACK 携带 8 个不同 decode_prefix_len（实测 `[19456×7, 486400]`）→ sender 页预算与高 prefix rank 的 dst_kv_indices 长度错位 → IndexError 杀 transfer_worker → 队列孤儿 → 静默冻结**。分歧本身是 per-rank radix 架构固有属性（不是 bug），**bug 是 PD 协议假设 8 ACK 同值**。修复四层：PD-PFX-MIN（min 跨 8 ACK）+ XFER-RADIX-TRUNC（src>dst 截尾，高位 rank radix 已覆盖前段）+ MISMATCH/WORKER-ERROR 兜底（fail 单请求不杀线程）+ `pop_decode_prefix_len` 的 `.pop()`→`.get()`（**次生 bug**：finalize_bootstrap 在首 chunk 入队前 pop 掉 entry，transfer_worker 的 per-rank offset 读恒 None——13,931/13,931 XFER-OFFSET-DBG 实证）。判据：`PD-PFX-MIN`（分歧检测）、`XFER-RADIX-TRUNC`（截断触发=防护生效）、`XFER-OFFSET-DBG` 的 room_min（.get 修复后非 None）。**冷启动注记：decode 重启后 radix 冷（1102 rid×8 rank 全一致、PD-PFX-MIN=0）——分歧需 L1 填满+驱逐漂移后出现（上次 2.5h/312 次），"零分歧"只代表树未漂**。
- **⛔ decode HiCache restore FAILED 单方面 abort → batch 分歧 → NCCL watchdog SIGABRT（2026-09-03 修复）**：per-rank L2 恢复失败（TP0 的 L2 在 match 后丢页：`load_back failed l1=26880 l2=323072 new=0`，TP1-7 同请求成功）时，`HiCacheRestoreGatedKVReceiver.poll` 只 gate PENDING **漏了 FAILED**（返回原始 poll），而 `pop_transferred` 的本地 `or hicache_restore_status == FAILED` **单方面 abort**（不走集体决议）→ TP0 队列 19 vs TP1-7 队列 20（`[PADDED-AR] len=19 vs 20` 铁证）→ TP1-7 COMMIT 请求进 batch 而 TP0 没有 → EAGLE verify 的 `eagle_sample` broadcast（PG2）跨 rank batch 不齐（rank0 走到 SeqNum 190399、rank1-7 卡 190393）→ **NCCL 600s watchdog → SIGABRT（exit -6）→ 整 decode 死**。与 2026-08-24 DSPark batch 分歧同构（当时只给 DSPark 加了 `_assert_batch_bs_rank_invariant`，EAGLE 路径的分歧入口是 restore FAILED）。修复三件套：①gate 把 FAILED 映射 `KVPoll.Failed`（=0 是 all_reduce min 最强传播，同一迭代 8 rank 统一 abort）②pop_transferred 只信集体决议（删单方面 or-FAILED）③`_process_hicache_local_restores` 异常降级 PENDING→FAILED（原 READY 会带 `hicache_restored_node=None` 走到 COMMIT AttributeError）。判据：L2 恢复失败后 8 rank 同刻 `Decode transfer failed ... uniform abort via gated-poll all_reduce min` + `[PADDED-AR]` len 跨 rank 恒等 + flood 单请求 503 但继续（~1/77min 频率）；**复发形态对照：len 分歧/部分 rank COMMIT → NCCL watchdog SIGABRT**。崩溃现场保存：1104 `/root/glm53_decode.log.crash-0903`。
- **⛔ decode radix finish 插 output = 死体积 churn → L2/restore 风暴 → 在途 KV 污染（2026-09-05 修 `8cd8707855`，机制形式化 2026-09-07 `docs/agent/decode-radix-output-churn-kv-pollution.md`）**：`release_kv_cache` 把 `kv_len_to_handle=effective_kv_committed_len`（prefix+output）传给 `cache_finished_req` → 每完成请求把 ~E[|output|]（GLM-5.3 thinking 2K-8K）**永无 match 的无锁可驱逐叶子**塞进 decode radix（agent replay 重写最后一条消息，output 链不复用；共享前缀走既有路径几乎不产生新节点）。**插入本身槽位自洽**（dup-free 只碰自己页/restore 段有 cache_protected_len 守卫/EAGLE 未提交段被排除）——污染经中间机制：**死体积 → admission 预算靠驱逐兑现（`available+evictable` 不变但成本变 write_back 链）→ 大 batch 同刻完成 = k·|O| 死体积入树 + k 次 dec_lock（共享前缀最后持有者完成→解锁可驱逐）+ 分配需求持续 → evict 风暴 → L2 write_back 淹没 host 池 → 驱逐越过冷死体积累触共享前缀 → L1→L2 降级风暴 → 后续 bootstrap needs_restore 突发（152/min 实证对齐 accept 悬崖）→ L2 restore 竞争窗口族（W1-W5：同节点并发 restore 4889268bbb / abort 漏放 host 锁 c23f23fa03 / DCP 轮换 58307d33e9 / read-before-ready 7da1b4e9b8 / 残余 L1-drift）任一命中 → dangling/提前释放槽位或污染内容 → 在途请求 target verify 读污染 KV → 乱码 + accept 0.26→0.00-0.09**。**修复=钳到 input 消灭压力源**（P(污染)∝风暴率∝λ·E[|O|]，钳制后死体积=0、完成时 free list +k·|O| 反向补充、共享前缀稳留 L1）——与 PFX-SYNC 同哲学（关源不关窗口；同日 W1-W4 已修但窗口仍在，风暴率未除所以 8cd8707855 仍必要）。**⚠️ 修复副作用（未修）：`release_kv_cache` 的 `start_p` 未随 `_insert_len` 钳制 → output 整页区间 `(P(I),P(C))` 无人释放（`free(kv_indices[page_aligned_len:])` 只盖尾页 + `_release_overallocated` 只盖 `[⌈C/64⌉,alloc)`）→ 每完成请求泄漏内页 ≈max(0,|O|−512) token（output≤512 被输入尾页+committed 页两块边界页吸收=零泄漏，长尾按内页数）。**spot b300-2 日志实证（2026-09-07，6552 请求/105min）**：96.3% 输出≤500（零泄漏人口），4% 长尾贡献泄漏率 **+19~22K token/min**（2681 样本匹配态 OLS γ=+22,052 tokens/min，z≈+10.7；10 组 running×prealloc 匹配组早/晚对照全 +14~26K/min），105min 累积 **+2.31M=21.3% 池** → evictable 6.86M 被挤压 **≈6h 耗尽 radix 容量**（慢性：树萎缩→命中率崩→L2 churn→OOM retract；若换 thinking 长输出流量 avg>512 泄漏率 ×30+ 成急性）。**修复（`fix/decode-radix-release-leak`，2026-09-07 已实施，四件套——朴素只钳 start_p 不够，retract 路径会双 free 边界页）**：① `start_p, end_p = _insert_len, kv_allocated_len`（output 区落入 overallocated 释放，镜像 #22373）；② `_release_overallocated_kv_indices` 对齐粒度改树页 `tree_cache.page_size`（DCP=256 vs flag=64）——朴素钳制后 ceil64 落 256 页中间 → retract 路径（无 free_group 去重）与树尾 free 重叠释放 P(I) = 双 free → 树页对齐后 floor/ceil 结构化不相交（finish 路径 free_group flush 的 unique 兜底亦在）；③ 非 spec assert `==`→`<=`；④ radix_cache disabled 分支删 `kv_allocated_len=kv_len_to_handle` 钳制（8cd8707855 后使 disabled 路径恒空泄漏；unified 的 disabled 分支无此行不受影响）。**验证**：py_compile + 页覆盖算术仿真 2880 组合 × 10 场景（FIX 零 gap 零双 free / OLD 复现泄漏 11-93 页·请求⁻¹ / NAIVE 无对齐复现 retract 双 free P(I)）——朴素修法必死的证明。判据（部署修复后复测）：匹配态回归斜率 used~running+pre_tokens+γ·t 的 γ 应 <2K/min（噪声底）；`sglang:kv_used_tokens` 在匹配 (running,prealloc) 态的单调漂移=复发；`HC-LOADBACK-DEDUP`/`HC-RESTORE-L1-DRIFT` 突发计数=风暴回归信号；needs_restore 突发对齐 accept 悬崖=此病；`[full]` available_size 单调下降=leak 复现。

- **⛔ LoRA 远程缓存必须内容寻址 + 有生命周期（2026-09-15）**：`resolve_lora_local_path` 旧 key = `basename + sha1(完整URL)[:8]`，而预签名 URL 每次请求都换 `X-Amz-Signature/Date/Expires` ⇒ **`.done` 永不命中，同一 adapter 反复全量下载+解压**（1022 实测：同一 UUID 存在 10/7/6/4 份目录；单次传输 11–12 s、解压 ~20 s、每次 15.2 GB）；且缓存**无回收**（4.8 TB 缓存 → 根分区 97%）。修复 = 新模块 `srt/lora/lora_cache.py`：key 取 `scheme://host/path`（丢签名）并按 artifact basename **回扫复用旧目录**；卸载写 `.unloaded_at`，超 `SGLANG_LORA_CACHE_TTL_SEC`（默认 24 h）才删；重复副本只保留最近使用的一份。**守卫（一律不删）：`in_use` 集合、`<dir>.lock`（下载在飞）、TTL 内被 touch 过**。开关 `SGLANG_LORA_CACHE_GC` / `SGLANG_LORA_CACHE_DEDUPE`；单测 `test/srt/lora/test_lora_cache.py`。判据：日志出现 `already cached ... dedup hit`，且同一 artifact 不再重复 `downloading lora`。详见 `docs/agent/lora-cache-gc-dedup.md`。

- **⛔ KV host-offload 索引必须走 DCP 域换算（2026-09-16 事故，`fix/kv-offload-index-domain`）**：decode retract 路径 `Req.offload_kv_cache`（schedule_batch.py:1629）把**裸 `req_to_token`** 交给 `allocator.get_cpu_copy` → `MLATokenToKVPool.get_cpu_copy` 直接 `kv_buffer[layer_id][chunk_indices]`。DCP 下 `req_to_token` 存的是**虚拟 id**（`size × dcp` 空间），物理缓冲只有 `size + page_size` 行 ⇒ **`id >= size` 即 CUDA device-side assert**（设备态 assert 无法被 try/except 捕获，毒化 context → **8 rank 全 abort → watchdog kill_process_tree → 引擎死**；08:23:32Z 实测，`dmesg` 8 卡同秒 Xid 43）。修复三件套：① 池层新增 `localize_kv_offload_indices()`（virtual→local + owner 过滤，非 owner/非法→scratch 行 `self.size`，与 `_write_mla_kv_buffer` 逐字一致）② `assert_kv_offload_indices_in_range()` 越界即抛 `KvOffloadIndexError` + `OFFLOAD-OOB[...]` 日志（把设备态 assert 变成可捕获的 Python 异常）③ `Req.offload_kv_cache/load_kv_cache` 包 try/except 降级（`OFFLOAD-SKIP` + `kv_cache_cpu=None`，offload 只是"省重算"优化，**永不带崩引擎**）。判据：`grep -cE "OFFLOAD-OOB|OFFLOAD-SKIP"` 应为 0（正常路径）；`grep -c "device-side assert"` 应为 0；单测 `test/srt/test_kv_offload_indices.py`。⚠️ 同类坑家族：**任何拿分配器 id 直接索引物理池的新路径都必须先做域换算 + 越界守卫**（另见 §8 的 `_guard_kv_indices` 用 SWA 尺寸当 FULL cap、`59c8532d79` off-by-one 超域页）。详见 `docs/agent/kv-offload-index-domain.md`。
...
| `docs/agent/decode-hicache-read-before-ready.md` | **read-before-ready：decode HiCache L2 load_back 树 value 提交先于 DMA（2026-08-31 `7da1b4e9b8`）**：`load_back()` 在 DMA 排队后同函数内 commit 树 value → match 立即可见 → 并发请求命中 pending 槽位读未完成 DMA。修复=forward stream 级 wait-all-in-flight。decode HiCache 乱码三重叠加的另一层（guard 洗 0 + read-before-ready + Triton 编译死锁） |
| `docs/agent/mol-lora-stage-cfg.md` | **MoL LoRA stage 调参（`SGLANG_OPT_MOL_LORA_STAGE_CFG`，2026-09-15）**：虚拟专家 LoRA shrink/expand 复用 MoE stage config，fallback（`BLOCK_SIZE_M=64/BLOCK_SIZE_N=64`）把 align padding 放大 ~93×（~1.1k 真实 pair → ~1e5 padded rows）→ down-delta expand `grid=[153984]` **160.6 µs/层 ≈ 14% 步时 @ ~85 GB/s ≈ HBM 峰值 1%**；三处 grid 独立对账证明 `grid = blocks × cdiv(N, BLOCK_SIZE_N)`；**修法只调 `BLOCK_SIZE_N`（min(256, stage 真实 N)），逐位等价**，⛔ 不动 `BLOCK_SIZE_M`（`grid_m ≡ blocks` 与该值无关）；含 A/B 验收协议（同 bs 同 60 步窗口、`args.grid` 判据、cases_50、`MOL-LORA-STAGE-CFG` 日志判据） |
| `docs/agent/pfx-sync-source-elimination.md` | **PFX-SYNC：跨 rank prefix min 协商——per-rank radix 分歧源头消灭（2026-09-03 `3812e4febb`）**：decode 8 rank 在任何 ACK 前集体协商 min(total_prefix_len)（all_gather 毕业 + sticky min + 两阶段 prealloc + 钳位），分歧段重收——传输层（offset/TRUNC/MISMATCH）全部变死代码。含 budget 断裂锁保留（cached match 跨等待持锁防 KV 损坏）+ FINISH_ABORT 泄漏修复 + 补偿路径。判据：PD-PFX-MIN 永不再触发 + `[PFX-SYNC] clamped` 替代 MISMATCH/WERR 503 |
| `docs/agent/pd-rank-divergence-root-cause.md` | **PD KV 传输 src/dst 错位根因：decode TP rank radix 命中分歧（2026-09-02 结案）**：`decode_prefix_len = L1+L2` 是 per-rank 本地状态（L1 驱逐/L2 write-back/L2→L3 逐出均 per-rank），8 ACK 各带各值 → sender 页预算 vs per-rank dst 数组错位 → IndexError 杀 worker → 冻结。修复四层：PD-PFX-MIN（min 跨 8 ACK）+ XFER-RADIX-TRUNC（src>dst 截尾）+ 兜底 fail 单请求 + `pop_decode_prefix_len` .pop→.get（offset 路径复活）。含"分歧是架构固有、bug 是协议假设同值"的定性 + 冷启动注记（分歧需驱逐压力积累） |
| `deploy/glm53-2p2d/deploy_2p2d.sh` + `docs/agent/glm53-2p2d-deploy.md` | **GLM-5.3 2P2D（38.255.28.x B200）+ 1102/1104（B300）部署/恢复手册（2026-08-30）**：五分支部署脚本（prefill/decode/router/storefix/storecreate + docker create→tar overlay→start 三段式）；5 个实录坑——引擎 mooncake env 不全=InnerTransferEngine cast 崩（22 项 common_env 一个不能少）、store daemon CMD 数组格式=bash 只执行 "exec" 秒退 exit 0、store 缺 --gpus all=libcuda ImportError、store 缺 offload 目录=FileStorage ValueError exit 1、daemon 脚本需 volume 挂载；1102 hasattr 3 参数 L2 write-back 驱逐崩溃（`0b589a9b19`）+ 裸机对恢复 runbook（tar 覆盖 venv + 清 pycache + 配对重启 + smg 31000）；快速恢复全流程（含 L3 store 在线扩容实录） |
| `docs/agent/decode-radix-output-churn-kv-pollution.md` | **decode radix finish 插 output → 在途 KV 污染的机制形式化（2026-09-07，修 `8cd8707855` 配套）**：插入本身槽位自洽（dup-free 只碰自己页/锁覆盖在途/守卫链完整）——污染经中间机制：死体积（output 叶永无 match）→ admission 预算靠驱逐兑现 → 大 batch 完成窗口三重耦合（k·\|O\| 死体积入树 + k 次 dec_lock 共享前缀解锁 + 分配持续）→ evict/write_back 风暴淹没 L2 → 共享前缀 L1→L2 降级 → needs_restore 突发（152/min 实证对齐 accept 悬崖）→ restore 竞争窗口族（W1-W5）任一命中 → 在途请求 target verify 读污染 KV（乱码+accept 0.26→0.00-0.09）。**修复=钳到 input 消灭压力源**（P(污染)∝风暴率∝λ·E[\|O\|]，与 PFX-SYNC 同"关源"哲学）。含不变量系统（I1 槽位活性/I2 锁覆盖/I3 restore 有效性）、排除法清单、**修复副作用：start_p 未随 _insert_len 钳制 → output 整页区间泄漏（≈\|O\|/256 页/请求，一行修复 `start_p=_insert_len` 镜像 #22373）** |
| `docs/agent/lora-cache-gc-dedup.md` | **LoRA 远程缓存：签名无关哈希去重 + 卸载后 24h GC（2026-09-15）**：预签名 URL 每次换 `X-Amz-*` ⇒ `sha1(完整URL)` key 永不命中 → 同一 adapter 重复全量下载（同一 UUID 10/7/6/4 份目录，单次 15.2 GB、传输 11–12 s + 解压 ~20 s）；缓存 4.8 TB → 根分区 97%。修复 = 新模块 `srt/lora/lora_cache.py`（key = `scheme://host/path` 哈希、basename 回扫复用旧目录、卸载 `.unloaded_at` + TTL(默认 24 h) GC、重复副本只留最新、残留归档/半成品回收），`lora_manager` 薄壳接线 + load 前/unload 后触发 sweep；守卫 in_use / `<dir>.lock` / TTL 内 touch；env `SGLANG_LORA_CACHE_{TTL_SEC,GC,DEDUPE}`；单测 `test/srt/lora/test_lora_cache.py`（本地与节点各 16/16，含 `tar --zstd` 分支） |
| `docs/agent/kv-offload-index-domain.md` | **KV host-offload 索引域崩溃（2026-09-16 结案，`fix/kv-offload-index-domain`）**：decode retract 路径 `Req.offload_kv_cache` 把裸 `req_to_token` 交给 `get_cpu_copy` → `kv_buffer[layer_id][chunk_indices]`；DCP 下该 id 是虚拟域（`size × dcp`）而物理池只有 `size + page_size` 行 ⇒ `id >= size` 触发 CUDA **device-side assert**（不可 try/except 捕获）→ 8 rank 全 abort → watchdog `kill_process_tree` → 引擎死（RL 侧只见 `engine_count=0` + 端点不可达）；`dmesg` 8 卡同秒 Xid 43。触发条件 = **KV 池打满（token usage 1.00）进 retract**，故长期潜伏。修复 = 池层 `localize_kv_offload_indices`（virtual→local + owner 过滤，非 owner/非法→scratch，与写路径逐字一致）+ `assert_kv_offload_indices_in_range`（`KvOffloadIndexError` + `OFFLOAD-OOB` 日志）+ `Req.offload/load_kv_cache` try/except 降级（`OFFLOAD-SKIP`，offload 只是省重算的优化）；单测 `test/srt/test_kv_offload_indices.py` 节点 7/7。含与 09-15 06:43Z 事故（池泄漏告警 + SIGTERM，**无** assert）的区分表 |
